      ...
      <motion.div initial={{ scale: 0.94, y: 16 }} animate={{ scale: 1, y: 0 }}
          ...
          <div style={{ position: "relative", flex: 1, minWidth: 0 }}>
            ...
            {results.length > 0 && (
              <div style={{ position: "absolute", top: "calc(100% + 5px)", left: 0, right: 0, zIndex: 20, background: "var(--bg-elevated)", border: "1px solid var(--border-default)", borderRadius: "12px", overflow: "hidden", boxShadow: "0 12px 30px rgba(0,0,0,0.25)" }}>
                {results.map(w => (
                  <button key={w.id} onClick={() => { onAddWord(w.id); setSearch("") }}
                    ...
                    <span style={{ color: "var(--accent-primary)", fontFamily: "var(--font-body)", fontSize: "12px", fontWeight: 700 }}>เพิ่ม</span>
                  </button>
                ))}
              </div>
            )}
        ...
        {loading ? (
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(220px,1fr))", gap: "9px" }}>
            {words.map((w, index) => (
              <div key={w.id} style={{ border: "1px solid var(--border-default)", background: "var(--bg-surface)", borderRadius: "12px", padding: "10px 11px", display: "flex", alignItems: "flex-start", gap: "9px" }}>
